...
{	//con esto encontramos la primera colision vertical
	...
	pos->rays.xintercep += pos->rays.isfacingright ? pos->tile.size : 0;
	...
	pos->rays.xstep *= pos->israyfacingleft ? -1 : 1;
	...
	pos->rays.nextverttouchy >= 0 && pos->rays.nextverttouchy <= pos->winres.window_height)
	{
		...
		{
			pos->raysvertwallhitx = pos->rays.nextverttouchx;
			pos->raysvertwallhity = pos->rays.nextverttouchy;
			pos->rays.vertwallcontent = ft_atoi(pos->map[(int)floor(pos->rays.ytocheck / pos->tile.size)][(int)floor(pos->rays.xtocheck / pos->tile.size)]);
...
{	//con esto encontramos la primera colision horizontal
	...
	pos->rays.yintercep += pos->rays.isfacingdown ? pos->tile.size : 0;
	...
	pos->rays.ystep *= pos->israyfacingup ? -1 : 1;
	...
	pos->rays.nexthoriztouchy >= 0 && pos->rays.nexthoriztouchy <= pos->winres.window_height)
	{
		...
		if (wall_colision(pos, pos->rays.ytocheck, pos->rays.xtocheck))
		{
			pos->rayshorizwallhitx = pos->rays.nexthoriztouchx;
			pos->rayshorizwallhity = pos->rays.nexthoriztouchy;
			pos->rays.horizwallcontent = ft_atoi(pos->map[(int)floor(pos->rays.ytocheck / pos->tile.size)][(int)floor(pos->rays.xtocheck / pos->tile.size)]);
